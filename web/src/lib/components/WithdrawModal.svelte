<script lang="ts">
	...
	function validateField(field: string, value: any) {
		...
		try {
			const fieldSchema = withdrawSchema.shape[field as keyof typeof withdrawSchema.shape];
			if (fieldSchema) {
			...
			<div>
				...
				<select
					...
					class="mt-1 w-full border px-3 py-2 text-sm text-slate-100 focus:outline-none {errors.accountId
						...
						: 'border-slate-700 bg-slate-800 focus:border-red-500'}"
				>
	...
	{#snippet footer()}
		<div class="flex justify-end gap-2">
			...
			<button
				...
				class="bg-red-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-50"
			>
